...
public class MyBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage() && update.getMessage().hasText()) {
            ...
            // Просмотр словаря
                ...
                // Берем список слов ИМЕННО этого юзера
                List<Word> myWords = userWords.getOrDefault(chatId, new ArrayList<>());

                if(myWords.isEmpty()) {
                    sendResponse(chatId, "В твоем словаре пусто! Добавь что-нибудь через кнопку «Добавить слово ➕»");
                } else {
                    StringBuilder wordResponse = new StringBuilder("Твой личный словарь:\n");
                    for(int k = 0; k < myWords.size(); k++) {
                        wordResponse.append(k + 1).append(". ").append(myWords.get(k).word).append(" - ").append(myWords.get(k).translation).append("\n");

                        // Если накопилось 40 строк, отправляем, чтобы не превысить лимит Telegram
                        if ((k + 1) % 40 == 0) {
                            sendResponse(chatId, wordResponse.toString());
                            wordResponse = new StringBuilder("Продолжение твоего словаря:\n");
                        }
                    }
                    // Отправляем остаток слов
                    if (wordResponse.length() > 25) {
                        sendResponse(chatId, wordResponse.toString());
                    }
                }
            }
    ...
    private void loadFromFile() {
        try {
            ...
            // Очищаем перед загрузкой, чтобы не было дублей при перезапуске
            userWords.clear();
